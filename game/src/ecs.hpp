...
struct ECS {
  ...
  void add_tracker(handler_id& handler, float x, float y, float r) noexcept {
    handler.tracker_id = static_cast<uint16_t>(trackers.size());
  ...
  void add_dimetions(handler_id& handler, float w, float h) noexcept {
    handler.obj_size_id = static_cast<uint16_t>(object_sizes.size());
    object_sizes.emplace_back(w, h);
  }

  void add_drag(handler_id& h) noexcept {
    h.drag_id = static_cast<uint16_t>(drags.size());
  ...
  void add_texture(handler_id& handler, uint16_t texture_id, float w, float h) noexcept {
    ...
    handler.tex_size_id = static_cast<uint16_t>(texture_sizes.size());
  ...
  void make_movable(handler_id& h) noexcept {
    h.motion_id = static_cast<uint16_t>(motions.size());
  ...
  void loop_logic(game_state& state) noexcept {
    if (state.frame_counter >= state.next_blink_frame) {
      ...
      state.next_blink_frame = static_cast<uint16_t>(state.frame_counter + delay);
    }
  ...
  void move() noexcept {
    static constexpr float dt = 1.0f / static_cast<float>(kScreenFps);
    for (auto mv : movs) {
