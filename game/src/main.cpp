...
int main(int /*argc*/, char* /*args*/[]) {
  ...
  ecs.add_texture(reye_id, static_cast<uint16_t>(manager.get_texture_id("right_eye")), 100, 100);
  ecs.add_tracker(reye_id, 462, 335, 23);

  auto head_id = ecs.register_object(center_x, center_y + 180);
  ecs.add_texture(head_id, static_cast<uint16_t>(manager.get_texture_id("head0_256")), 512, 512);
  ecs.add_tracker(head_id, center_x, center_y + 80, 10);
  state.head_texture_next = static_cast<uint16_t>(manager.get_texture_id("head1_256"));
  ...
  ecs.add_texture(table_id, static_cast<uint16_t>(manager.get_texture_id("table")), 800, 200);

  auto score_id = ecs.register_object(122, 38);
  ecs.add_texture(score_id, static_cast<uint16_t>(manager.get_texture_id("score")), 224, 56);
